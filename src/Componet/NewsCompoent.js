...
export class NewsCompoent extends Component {
    // articles = [
    //     {
    //         "source": {
    //             "id": "bbc-news",
    ...
    constructor() {
        super();
        console.log("i am shuper constructoue componet")
        this.state = {
            articles: [],
            page: 1,
            loading:false
    ...
    async componentDidMount() {
        let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&category=${this.props.category}&apiKey=2e00eda08c24411bb8af920f06a81d50&pageSize=${this.props.pageSize}`

        let data = await fetch(url);
        let parsdat = await data.json()
        console.log(parsdat);
        this.setState({
            articles: parsdat.articles,
            totalResults:parsdat.totalResults
        })
    }
    Prviewbutton = async () => {
        console.log("prview")
        let url = `https://newsapi.org/v2/top-headlines?country=${this.props.country}&apiKey=2e00eda08c24411bb8af920f06a81d50&page=${this.state.page-1}&pageSize=${this.props.pageSize}`
    ...
    render() {
        return (
            <div className='container my-5'>
                <div className='row' >
                    <h1>News Monkey - Top Hedlines!</h1>
                    
                    {this.state.articles.map((element) => {
                        return <div className='col-md-4 d-flex justify-content-center'>
                            <NewsIteam title={element.title} description={element.description} imageurl={element.urlToImage} Url={element.url} author={element.author} publishedAt={element.publishedAt}/>
                        </div>
                    })}

                </div>
                <div className='container d-flex justify-content-between'>
                    <button disabled={this.state.page<=1} type="button" className="btn btn-dark" onClick={this.Prviewbutton} > &larr; prview</button>
...
export default NewsCompoent
//disabled={this.state.totalResults/this.props.pageSize}